  ...
  const loadProfile = async () => {
    try {
        ...
        if (decoded || freshUser) {
          setProfile({
            ...
            address:
              ...
              "N/A",
            orgId:
              freshUser?.orgId || decoded?.orgId || decoded?.OrgId || "N/A",
            ...
            profileImage: getImageUrl(
              freshUser?.profileImage || decoded?.profileImage || null,
            ),
          });
        }
      }
      ...
      if (userData) {
        ...
        if (storedImage) {
          const imageUrl = getImageUrl(storedImage);
          if (imageUrl) {
            setProfile((prev) =>
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.user) {
          ...
          const imagePath = data.user.ProfileImage || data.user.profileImage;
          const updatedImage = getImageUrl(imagePath);

          if (updatedImage) {
            setProfile((prev) =>
              prev ? { ...prev, profileImage: updatedImage } : null,
            );
          }
        }
