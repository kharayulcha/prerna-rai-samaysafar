...
export default function Dashboard() {
      ...
      if (token) {
        ...
        if (userDataString) {
          try {
            ...
            profileImageFromData = userData.ProfileImage || userData.profileImage;
          } catch (e) {
        ...
        if (decoded) {
          ...
          const userName =
            ...
            "Welcome User";
          const userEmail = freshUser?.email || decoded.email || decoded.Email || "";
          const userPhone =
            freshUser?.phone ||
            decoded.phone ||
            decoded.Phone ||
          ...
          const userRole = (freshUser?.role || decoded.role || decoded.Role || "user").toLowerCase();
          const parentName = freshUser?.parentName || decoded.parentName || null;
          ...
          const driverName = freshUser?.driverName || decoded.driverName || null;
          const driverPhone = freshUser?.driverPhone || decoded.driverPhone || null;

          // Determine logo source - prefer stored base64 (from either key), fallback to default
          let logoSource: any = require("../assets/images/logo.png");
          if (storedLogo && storedLogo.startsWith("data:")) {
            logoSource = { uri: storedLogo };
          } else if (profileImageFromData && profileImageFromData.startsWith("data:")) {
            logoSource = { uri: profileImageFromData };
          }
        ...
        if (status !== "granted") {
          ...
          const subscription = await Location.watchPositionAsync(
            ...
            async (locationResult) => {
              ...
              //via Socket.IO for instant broadcast + DB persistence (handled in app.ts)
              try {
              ...
              //  Also POST to backend so location is persisted in DB
              try {
  ...
  const quickActions = [
    ...
    { id: 8, name: "Trip Log", icon: "document-text" as const, color: "#5FA3E3BD" },
              ...
              <View style={styles.idCardHeader}>
                <View style={styles.logoContainer}>
                  <Image
                    ...
                    resizeMode="cover"
                    onError={(e) => {
                      console.log("[Dashboard] Student logo load error:", e.nativeEvent.error);
                      setOrganizationData(prev => ({ ...prev, logo: require("../assets/images/logo.png") }));
                    }}
                  />
                </View>
                    ...
                    cells.push(
                      ...
                      >
                        ...
                        {hasStartTime && (
                          ...
                          >
                            {formatTimeStr(organizationData.startTime || "07:30")}
                          </Text>
                        )}
                      </View>,
                    );
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { flex: 0.9 }]}>
            ...
            {isFetchingReport ? (
              <ActivityIndicator size="large" color="#4FA3FF" style={{ flex: 1 }} />
            ) : (
              <>
                    ...
                    <ScrollView showsVerticalScrollIndicator={true}>
                      <View style={styles.reportTable}>
                        <View style={styles.reportTableHead}>
                          {reportType === "trips" ? (
                            <>
                              <Text style={[styles.reportTh, { width: 40 }]}>#</Text>
                              <Text style={[styles.reportTh, { width: 100 }]}>Date</Text>
                              <Text style={[styles.reportTh, { width: 80 }]}>Start</Text>
                              <Text style={[styles.reportTh, { width: 80 }]}>End</Text>
                              <Text style={[styles.reportTh, { width: 120 }]}>Route</Text>
                              <Text style={[styles.reportTh, { width: 100 }]}>Bus</Text>
                              <Text style={[styles.reportTh, { width: 120 }]}>Driver</Text>
                              <Text style={[styles.reportTh, { width: 80 }]}>Status</Text>
                            </>
                          ) : (
                            <>
                              <Text style={[styles.reportTh, { width: 40 }]}>#</Text>
                              <Text style={[styles.reportTh, { width: 100 }]}>Date</Text>
                              <Text style={[styles.reportTh, { width: 150 }]}>Student</Text>
                              <Text style={[styles.reportTh, { width: 150 }]}>Parent</Text>
                              <Text style={[styles.reportTh, { width: 100 }]}>Method</Text>
                              <Text style={[styles.reportTh, { width: 100 }]}>Amount</Text>
                            </>
                          )}
                        </View>
                        {reportData.map((item, idx) => (
                          <View key={idx} style={[styles.reportTableRow, idx % 2 === 0 && { backgroundColor: "#F8FAFC" }]}>
                            {reportType === "trips" ? (
                              <>
                                <Text style={[styles.reportTd, { width: 40 }]}>{item.sn}</Text>
                                <Text style={[styles.reportTd, { width: 100 }]}>{item.date}</Text>
                                <Text style={[styles.reportTd, { width: 80 }]}>{item.start}</Text>
                                <Text style={[styles.reportTd, { width: 80 }]}>{item.end}</Text>
                                <Text style={[styles.reportTd, { width: 120 }]}>{item.route}</Text>
                                <Text style={[styles.reportTd, { width: 100 }]}>{item.bus}</Text>
                                <Text style={[styles.reportTd, { width: 120 }]}>{item.driver}</Text>
                                <Text style={[styles.reportTd, { width: 80, fontWeight: 'bold' }]}>{item.status}</Text>
                              </>
                            ) : (
                              <>
                                <Text style={[styles.reportTd, { width: 40 }]}>{item.sn}</Text>
                                <Text style={[styles.reportTd, { width: 100 }]}>{item.date}</Text>
                                <Text style={[styles.reportTd, { width: 150 }]}>{item.student}</Text>
                                <Text style={[styles.reportTd, { width: 150 }]}>{item.parent}</Text>
                                <Text style={[styles.reportTd, { width: 100 }]}>{item.method}</Text>
                                <Text style={[styles.reportTd, { width: 100, fontWeight: 'bold' }]}>Rs {item.amount}</Text>
                              </>
                            )}
                          </View>
                        ))}
