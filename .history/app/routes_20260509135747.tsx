...
const deg2rad = (deg: number) => deg * (Math.PI / 180); // degree lai radians ma convert garne kam garxa


const getDistanceFromLatLonInKm = (lat1: number, lon1: number, lat2: number, lon2: number) => {
    const R = 6371; //earth ko radius in km ma
    const dLat = deg2rad(lat2 - lat1); //converted to radians
    const dLon = deg2rad(lon2 - lon1); //converted to radians
    const a =
