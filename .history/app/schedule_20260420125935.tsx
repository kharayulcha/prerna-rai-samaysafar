  ...
  const handlePlaceSearch = (text: string) => {
    ...
    searchTimer.current = setTimeout(async () => {
      try {
        ...
        const res = await fetch(`${API_BASE_URL}/api/routes/utility/search-places?q=${encoded}`);
        const data = await res.json();
        const mappedResults = (data.features || []).map((f: any, idx: number) => {
              ...
              <View style={styles.assignedRouteCard}>
                ...
                <View style={styles.routeDetails}>
                  ...
                  <View style={styles.detailItem}>
                    ...
                    <Text style={styles.detailText}>
                      Default Start: {formatTimeStr(assignedRouteInfo.startTime)}
                    </Text>
                  </View>
                </View>
                    ...
                    {studentStops
                      ...
                      .map((stop: any, index: number) => (
                        <View key={stop.id || index} style={styles.etaRow}>
                          ...
                          <Text style={styles.etaTime}>Stop {stop.sequenceOrder}</Text>
                        </View>
                      ))}
                  </View>
                )}
          ...
          renderItem={({ item }) => (
            <View style={styles.scheduleItem}>
              ...
              <View style={styles.scheduleInfo}>
                <Text style={styles.scheduleTime}>{formatTimeStr(item.time)}</Text>
                    ...
                    <View style={styles.mapContainer}>
                      ...
                      >
                        {landmarks.length > 1 && (
                          <Polyline
                            coordinates={landmarks
                              ...
                              .sort((a: any, b: any) => a.SequenceOrder - b.SequenceOrder)
                              .map((lm: any) => {
                                const isEditing = (lm._tempId || String(lm.StopId)) === editingLandmarkId;
                                return {
                                  latitude: isEditing && newLandmarkLat ? parseFloat(newLandmarkLat) : parseFloat(lm.Latitude),
                                  longitude: isEditing && newLandmarkLng ? parseFloat(newLandmarkLng) : parseFloat(lm.Longitude)
                                };
                              })
                            }
                            strokeColor="#3B82F6"
                          ...
                          return (
                            ...
                            >
                              <View style={{ alignItems: 'center' }}>
                                <View style={{ backgroundColor: "#3B82F6", borderRadius: 12, width: 24, height: 24, justifyContent: 'center', alignItems: 'center', borderWidth: 2, borderColor: "#FFF", elevation: 5 }}>
                                  <Text style={{ color: "#FFF", fontWeight: 'bold', fontSize: 12 }}>{lm.SequenceOrder}</Text>
                                </View>
                              </View>
                            </Marker>
                          );
                        ...
                        {newLandmarkLat && newLandmarkLng ? (
                          ...
                          >
                            <View style={{ alignItems: 'center' }}>
                              <View style={{ backgroundColor: "#ef4444", borderRadius: 12, width: 24, height: 24, justifyContent: 'center', alignItems: 'center', borderWidth: 2, borderColor: "#FFF", elevation: 5 }}>
                                <Text style={{ color: "#FFF", fontWeight: 'bold', fontSize: 12 }}>
                                  {editingLandmarkId ? landmarks.find((l: any) => (l._tempId || String(l.StopId)) === editingLandmarkId)?.SequenceOrder : landmarks.length + 1}
                                </Text>
                              </View>
                            </View>
                          </Marker>
