...
describe("Test 7 - Add Driver", () => {
  ...
  beforeAll(async () => {
    ...
    if (loginResponse.status !== 200) {
      console.error("Login failed before add driver test:", loginResponse.status, loginData);
    }
    authToken = loginData.token;

    // Try to create a route to use for the driver
    try {
      const routeRes = await fetch(`${API_BASE_URL}/api/routes/create`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
      ...
      if (contentType && contentType.includes("application/json")) {
        ...
        routeId = routeData.route?.id || routeData.route?.RouteId || 1;
      } else {
        console.warn("Routes endpoint not available, using default routeId: 1");
        routeId = 1;
      }
    } catch (err) {
      console.warn("Failed to create route, using default routeId: 1", err.message);
      routeId = 1;
    }
  });
