...
export const listBuses = async (req: Request, res: Response) => {
  try {
    ...
    const mappedBuses = buses.map((bus) => {
      ...
      const assignedRoute = bus.routeAssignments[0]?.route || bus.assignedUsers[0]?.assignedRoute || null;
...
export const getFleetStatus = async (req: Request, res: Response) => {
  try {
    ...
    const fleetStatus = buses.map((bus) => {
      ...
      const assignedRoute = bus.routeAssignments[0]?.route || assignedDriver?.assignedRoute || null;
