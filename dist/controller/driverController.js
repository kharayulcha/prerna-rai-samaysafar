...
export const createDriver = async (req, res) => {
    try {
        ...
        const { name, email, phone, password, routeId, busId } = req.body;
...
export const editDriver = async (req, res) => {
    try {
        ...
        const { name, phone, password, routeId, busId } = req.body;
...
export const deleteDriver = async (req, res) => {
    try {
        ...
        if (!driver || driver.OrgId !== orgId || driver.Role !== 'driver')
            return res.status(404).json({ message: 'Driver not found' });
...
export const listDrivers = async (req, res) => {
    try {
        ...
        const where = { OrgId: Number(orgId), Role: 'driver' };
        if (name)
        ...
        const mappedDrivers = drivers.map((driver) => {
            ...
            const directRouteName = driver.assignedRoute?.Name;
...
export const getDriver = async (req, res) => {
    try {
        ...
        if (!driver || driver.OrgId !== Number(orgId) || driver.Role !== 'driver')
            return res.status(404).json({ message: 'Driver not found' });
