    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (busIdsArr.length > 0) {
                ...
                if (buses.length !== busIdsArr.length)
                    throw new Error('Some buses not found or unauthorized');
            ...
            if (driverIdsArr.length > 0) {
                ...
                if (drivers.length !== driverIdsArr.length)
                    throw new Error('Some drivers not found or unauthorized');
...
export const deleteRoute = async (req, res) => {
    try {
        ...
        if (!route || route.OrgId !== Number(orgId))
