...
function AdminPaymentView() {
  ...
  const calculateTransportationFee = useCallback((route: AdminRoute, startDate: string, endDate: string): void => {
    try {
      ...
      // Calculate breakdown
      const breakdown = `Base: ₹${BASE_FEE} + Distance (${routeDistance}km × ₹${PRICE_PER_KM} = ₹${distanceFee.toFixed(0)}) = ₹${dailyFee.toFixed(0)}/day × ${daysInPeriod} days = ₹${periodFee}`;
...
function ParentPaymentView() {
  ...
  const [esewaHtml, setEsewaHtml] = useState<string>("");
...
.spinner{width:40px;height:40px;border:4px solid #ddd;border-top-color:#165C9C;border-radius:50%;animation:spin .8s linear infinite;margin:0 auto 16px}
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <View style={styles.card}>
                ...
                <TouchableOpacity
                  style={[styles.payButton, payingBillId === item.BillId && { opacity: 0.6 }]}
                  onPress={() => handlePayBill(item.BillId)}
                  ...
                  {payingBillId === item.BillId ? (
                    ...
                    <>
                      ...
                      <Text style={styles.payButtonText}>
                        Pay with eSewa ({formatCurrency(item.remainingAmount ?? item.Amount)})
                      </Text>
                    </>
                  )}
                </TouchableOpacity>
          ...
          <WebView
            ...
            renderLoading={() => (
              <View style={styles.webViewLoading}>
                ...
                <Text style={{ marginTop: 10, color: "#666" }}>Loading eSewa...</Text>
              </View>
            )}
          />
        </SafeAreaView>
