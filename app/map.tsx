...
import socket, { connectSocket, disconnectSocket, onLocationUpdate } from "../utils/socket";
...
export default function MapScreen() {
    ...
    const [locationTrail, setLocationTrail] = useState<{ latitude: number; longitude: number }[]>([]);
    ...
    const [liveRoutePath, setLiveRoutePath] = useState<{ latitude: number; longitude: number }[]>([]);
    const etaFetchTimer = useRef<ReturnType<typeof setTimeout> | null>(null);

    // Decode JWT to get user info
    const getUserInfo = async (): Promise<{ routeId: number | null; role: string | null; childrenRouteIds: number[] }> => {
        try {
    ...
    const fetchRoutes = useCallback(async () => {
        try {
            ...
            const token = await AsyncStorage.getItem("authToken");
            console.log("[MAP] Fetching routes from:", `${API_BASE_URL}/api/routes/get-routes`);
    ...
    const fetchActiveTrip = useCallback(async (rId: number) => {
        try {
            setError(null);
            const token = await AsyncStorage.getItem("authToken");
            ...
            try {
                const stopsRes = await fetch(`${API_BASE_URL}/api/routes/${rId}/details`, {
            ...
            console.log("[MAP] API URL:", `${API_BASE_URL}/api/trips/active/${rId}`);
            ...
            if (!res.ok) {
                ...
                console.warn("[MAP] No active trip. Server response:", res.status, errorText);
                setTripInfo(null);
                setBusLocation(null);
                setLocationTrail([]);
            ...
            console.log("[MAP] Latest location:", JSON.stringify(data.latestLocation));
            ...
            if (data.latestLocation) {
                ...
                console.log("[MAP] Setting initial bus location:", JSON.stringify(loc));
                setBusLocation(loc);
            } else {
    ...
    const fetchETAs = useCallback(async (busLat: number, busLng: number) => {
        ...
        try {
            ...
            const upcomingStops = sortedStops.filter(s => !passedStopIds.has(s.id));
            const etas: StopETA[] = [];
            ...
            if (upcomingStops.length > 0) {
                // Get the road-snapped route line (bus → stops in sequence for the polyline)
                const seqCoords = [
                    `${busLng},${busLat}`,
                ...
                const seqRes = await fetch(
                    `https://router.project-osrm.org/route/v1/driving/${seqCoords}?overview=full&geometries=geojson&steps=false`
                );
                ...
                if (seqRes.ok) {
                    ...
                    if (seqData.routes && seqData.routes.length > 0) {
                        if (seqData.routes[0].geometry) {
                            const roadCoords = seqData.routes[0].geometry.coordinates.map((c: number[]) => ({
                                latitude: c[1],
                                longitude: c[0],
                            }));
                ...
                for (const stop of upcomingStops) {
                    try {
                        ...
                        const pairRes = await fetch(
                            `https://router.project-osrm.org/route/v1/driving/${pairCoords}?overview=false&steps=false`
                        );
                        if (pairRes.ok) {
            ...
            etas.sort((a, b) => (stopOrder.get(a.stopId) ?? 0) - (stopOrder.get(b.stopId) ?? 0));
    ...
    const selectRoute = useCallback(async (rId: number) => {
        setRouteId(rId);
    ...
    useEffect(() => {
        ...
        const init = async () => {
                ...
                if (!rId) {
                    ...
                    if (userRole === "parent" && userInfo.childrenRouteIds.length > 0) {
                        if (userInfo.childrenRouteIds.length === 1) {
                            console.log("[MAP] Parent has 1 child route:", userInfo.childrenRouteIds[0]);
                            rId = userInfo.childrenRouteIds[0];
                        } else {
                            console.log("[MAP] Parent has multiple child routes, showing selector");
                            await fetchRoutes();
            ...
            cleanupLocationListener = onLocationUpdate((data) => {
                console.log("[MAP] >>> LIVE location-update received:", JSON.stringify(data));
                setBusLocation(data);
            ...
            socket.on("notification", (data: any) => {
                console.log("[MAP] >>> Socket notification received:", JSON.stringify(data));
            ...
            <View style={styles.mapContainer}>
                {loading ? (
                    ...
                    <View style={styles.routeSelectorContainer}>
                        ...
                        <Text style={styles.routeSelectorTitle}>Select a Route to Track</Text>
                        <Text style={styles.routeSelectorSubtitle}>
                            Choose a route to see the live bus location
                        </Text>

                        {loadingRoutes ? (
                            <ActivityIndicator size="large" color={DEEP_BLUE} style={{ marginTop: 20 }} />
                        ) : (
                            <FlatList
                    ...
                    <>
                                            ...
                                            <View style={{
                                                ...
                                                {eta && (
                                                    <Text style={{
                                                        ...
                                                        {eta.passed ? '✓ Passed' : `~${formatETA(eta.durationSeconds)}`}
                                                    </Text>
                                                )}
                                            </View>
                                            <View style={{
                        ...
                        <View style={styles.mapOverlayButtons}>
                            ...
                            {busLocation && (
                                <TouchableOpacity
                                    style={[styles.overlayButton, !followBus && styles.overlayButtonInactive]}
                                    onPress={centerOnBus}
                                >
                                    <Ionicons name="locate" size={22} color={followBus ? DEEP_BLUE : "#999"} />
                                </TouchableOpacity>
                            )}
                                ...
                                {stopETAs.length > 0 && (
                                    <View style={styles.etaSection}>
                                        ...
                                        <View style={styles.etaHeader}>
                                            <Ionicons name="navigate" size={14} color={PRIMARY_BLUE} />
                                            <Text style={styles.etaHeaderText}>Estimated Arrival</Text>
                                        </View>
                                        {stopETAs.map((eta) => (
                                            <View key={eta.stopId} style={styles.etaRow}>
                                                ...
                                                <Text
                                                    style={[
                                                        styles.etaStopName,
                                                        eta.passed && { color: '#aaa', textDecorationLine: 'line-through' }
                                                    ]}
                                                    numberOfLines={1}
                                                >
                                                    {eta.stopName}
                                                ...
                                                <Text style={[
                                                    ...
                                                    {eta.passed ? '✓' : `~${formatETA(eta.durationSeconds)}`}
                                                </Text>
                                            </View>
                                        ))}
                                    </View>
